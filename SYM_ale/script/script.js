...
var sortingInput = $('input[name="sorting"]'); // Input radio per l'ordinamento

// Funzione per ottenere i dati dell'API
function fetchMovies() {
    container.html("");

    var sorting = sortingInput.filter(":checked").val();
    console.log("Selected Sorting: " + sorting);


    var apiUrl = `https://api.themoviedb.org/3/${searchQuery !== '' ? 'search' : 'discover'}/movie?api_key=${APIKEY}&query=${searchQuery}&page=${pageNumber}&language=en-US`;
    if (sorting === 'popularity') {
        apiUrl += '&sort_by=popularity.desc';
    } else if (sorting === 'release') {
        apiUrl += '&sort_by=release_date.asc';
    }

    
    

    apiUrl += '&include_adult=false&include_video=false&with_watch_monetization_types=flatrate';
...
$('#releaseSorting').click(function() {
    console.log("Release Sorting Clicked");
    // Chiamata per recuperare i film con ordinamento "Released"
    fetchMovies();
});

$('#popularitySorting').click(function() {
    console.log("Release Sorting Clicked");
    // Chiamata per recuperare i film con ordinamento "Released"
    fetchMovies();
});
